...
@document_router.post("/upload/{project_id}", response_model=DocumentResponse)
async def upload_document_handler(
    ...
    print("/documents/upload/{project_id} POST route reached")
    return await upload_document_to_project(supabase, project_id, file, user_id)


@document_router.get("/project/{project_id}", response_model=List[DocumentResponse])
async def get_project_documents_handler(
    ...
    print("/documents/project/{project_id} GET route reached")
